 ...
 */
public class Online extends Fragment implements maprecycleclicklistener {

    String permission_str[]={Manifest.permission.CALL_PHONE};
    RecyclerView rc;
    RecyclerView.LayoutManager layoutManager;
    RecyclerView.Adapter adapter;
    List<schedule_list_element> online_lis=new ArrayList<>();
    ProgressBar prog;
    public Online() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        prog=(ProgressBar) RootView.findViewById(R.id.prog2);
        rc=(RecyclerView) RootView.findViewById(R.id.rec_cards2);
        layoutManager = new LinearLayoutManager(getContext());
        rc.setLayoutManager(layoutManager);
        rc.setVisibility(View.INVISIBLE);
        // specify an adapter (see also next example)
        adapter = new Schedule_RecyclerAdapter(online_lis,this);
        ...
        register_api_class.getClient().getevents(new Callback<schedule_pojo>() {
            ...
            public void success(schedule_pojo schedule_pojo, Response response) {
                Log.i("test","test");
                Day1.filtered_list(-1,schedule_pojo.getMessage(),online_lis);
    ...
    @Override
    public void recyclelistitemclick(int pos) {String phone = online_lis.get(pos).getEventContact().toString();
        String s = "tel:"+phone;
